...
function getverifycode1(field_id, field_name) {
  ...
  var sel_question =  frm.elements['sel_question'] ? Utils.trim(frm.elements['sel_question'].value) : '';
	...
	if(mobile == '') {
		layer.alert("- "+field_name+" 不能为空！");
		$(field_id).focus();
		return;
	}

	if(!frm.elements["captcha"]){
		layer.alert("请开启图形验证码功能(系统设置-验证码管理-勾选新用户注册-保存设置)！");
		return;
	}
	...
	if(smsyy==-1 || parseInt(smsyy)>0){
		// layer.alert('sms.php?step=getverifycode1&r=' + Math.random()+ 'mobile=' + mobile+'&smscode=' + smscode);
...
function RemainTime(){
    ...
    if (iTime >= 0) {
        ...
        if (iSecond >= 0) {
            if (iMinute>0) {
            	console.log(4);
                sSecond = "验证码(" + iMinute + "分" + iSecond + "秒)";
            }else{
                sSecond = "验证码(" + iSecond + "秒)";
            }
        }
        sTime = sSecond;
        if (iTime == 0) {
