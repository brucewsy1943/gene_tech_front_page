...
var MagicZoom_ua='msie';var W=navigator.userAgent.toLowerCase();if(W.indexOf("opera")!=-1){
    ...
    this.version='2.3';this.recalculating=false;this.smallImageCont=MagicZoom_$(smallImageContId);this.smallImage=MagicZoom_$(smallImageId);this.bigImageCont=MagicZoom_$(bigImageContId);this.bigImage=MagicZoom_$(bigImageId);this.pup=0;this.settings=settings;if(!this.settings["header"]){
        ...
        this.loadingCont=document.createElement('DIV');this.loadingCont.style.position='absolute';this.loadingCont.style.visibility='hidden';this.loadingCont.className='MagicZoomLoading';this.loadingCont.style.display='inline-block';this.loadingCont.style.textAlign='center';this.loadingCont.innerHTML=this.settings["loadingText"]+'<br/><img border="0" alt="'+this.settings["loadingText"]+'" src="'+this.settings["loadingImg"]+'"/>';this.smallImageCont.appendChild(this.loadingCont)
    ...
    var pleft=this.positionX-this.popupSizeX/2;var ptop=this.positionY-this.popupSizeY/2;var perX=pleft*(this.bigImageSizeX/this.smallImageSizeX);var perY=ptop*(this.bigImageSizeY/this.smallImageSizeY);if(document.documentElement.dir=='rtl'){
        ...
        }this.bigImage.style.left=(-perX)+'px';this.bigImage.style.top=(-perY)+'px';this.bigImageCont.style.top=this.bigImageContStyleTop;this.bigImageCont.style.display='inline-block';this.bigImageCont.style.visibility='visible';this.bigImage.style.display='inline-block';this.bigImage.style.visibility='visible';this.recalculating=false
    ...
    var loadingText='Loading Zoom';var loadingImg='';var iels=window.document.getElementsByTagName("IMG");for(var i=0;i<iels.length;i++){
        ...
        if(/MagicZoom/.test(aels[i].className)){
            while(aels[i].firstChild){
                ...
                }if(aels[i].firstChild.tagName!='IMG')throw"Invalid MagicZoom invocation!";var rand=Math.round(Math.random()*1000000);aels[i].style.position="relative";aels[i].style.display='inline-block';aels[i].style.outline='0';aels[i].style.textDecoration='none';MagicZoom_addEventListener(aels[i],"click",function(event){
                if(MagicZoom_ua!='msie'){
                    this.blur()
                    }MagicZoom_stopEventPropagation(event);return false
                });if(aels[i].id==''){
